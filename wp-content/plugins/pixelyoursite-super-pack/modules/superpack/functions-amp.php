...
function outputEvents() {
	...
	if ( PixelYourSite\Facebook()->configured() && ! apply_filters( 'pys_pixel_disabled', array(), 'facebook' ) ) {
		
		foreach ( $eventsManager->getStaticEvents( 'facebook' ) as $eventName => $events ) {
			foreach ( $events as $event ) {
                $ids = $event['pixelIds'];
				foreach ( $ids as $pixelID ) {
					renderFacebookTag( $pixelID, $event['name'], $event['params'] );
				}
			}
		}
	...
	if ( PixelYourSite\GA()->configured() && ! apply_filters( 'pys_pixel_disabled', array(), 'ga' ) ) {
		
		foreach ( $eventsManager->getStaticEvents( 'ga' ) as $eventName => $events ) {
			foreach ( $events as $event ) {
                $ids = $event['trackingIds'];
				foreach ( $ids as $pixelID ) {
					renderGoogleAnalyticsTag( $pixelID, $event['name'], $event['params'] );
				}
			}
		}
	...
	if ( PixelYourSite\Pinterest()->configured() && ! apply_filters( 'pys_pixel_disabled', array(), 'pinterest' ) ) {
		...
		foreach ( $eventsManager->getStaticEvents( 'pinterest' ) as $eventName => $events ) {
			foreach ( $events as $event ) {
				foreach ( PixelYourSite\Pinterest()->getPixelIDs() as $pixelID ) {
					renderPinterestTag( $pixelID, $event['name'], $event['params'] );
				}
			}
		}
