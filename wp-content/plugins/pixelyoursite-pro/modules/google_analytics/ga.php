...
class GA extends Settings implements Pixel {
    ...
    public function generateEvents($event) {
            ...
            && SuperPack()->getOption( 'enabled' ) )
        {
            ...
            if (Ads()->configured() && !$this->isOnlyGA4event($event->getId())){
            ...
            if(count($pixelIds) > 0) {
                ...
                $pixelEvent = clone $event;
                if(Ads()->configured() && !$this->isOnlyGA4event($pixelEvent->getId()) && $pixelEvent->getId() != 'custom_event') {
    ...
    private function getWooViewContentEventParams($eventArgs = null, $event = null)
    {
        ...
        if(!$this->isOnlyGA4event($event->getId()) && Ads()->configured() && Ads()->getAllPixelsForEvent($event)){
            $unifyAds = true;
        }
        $variable_id = null;
        $quantity = 1;
